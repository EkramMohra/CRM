...
class TopEmployees extends Component {
  ...
  async receivedData() {
    ...
    let owners = await this.props.c.owners;
  ...
  getTopEmployees = () => {
    ...
    for (let client of data) {
      TopEmployeesStore[client.owner_id] ?
        TopEmployeesStore[client.owner_id]++ : TopEmployeesStore[client.owner_id] = 1;
    }
    ...
    topEmployees.push({ name: owners[id].owner, sales: max });
    topEmployees.push({ name: owners[id_2].owner, sales: max_2 });
    topEmployees.push({ name: owners[id_3].owner, sales: max_3 });
...
export default inject("c")(observer(TopEmployees));
